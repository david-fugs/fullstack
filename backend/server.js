...
app.post("/api/orders", (req, res) => {
    ...
    // Formatear los artículos del carrito como una cadena
    ...
    const query =
      "INSERT INTO orders (name, id_number, address, total_price, items) VALUES (?, ?, ?, ?, ?)";
    db.query(
      query,
      [customer.name, customer.idNumber, customer.address, totalPrice, itemsDetails],
      (err, result) => {
        if (err) {
          console.error("Error al guardar la orden:", err);
          return res.status(500).json({ error: "Error al guardar la orden" });
        }
        res.status(200).json({ message: "Orden guardada con éxito" });
      }
    );
  });
...
app.post("/api/products", (req, res) => {
  ...
  const query =
    "INSERT INTO productos (name_product, code_product, description_product, price_product, url_image) VALUES (?,?,?,?,?)";
...
app.put("/api/products/:id", (req, res) => {
  const productId = req.params.id;
  const { name_product, description_product, price_product, url_image } =
    req.body;

  // Consulta SQL para actualizar el producto
  const query =
    "UPDATE productos SET name_product = ?, description_product = ?, price_product = ?, url_image = ? WHERE id_product = ?";
  db.query(
    query,
    [name_product, description_product, price_product, url_image, productId],
    (err, result) => {
      if (err) {
